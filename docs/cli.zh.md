...
| `arbor demo` | 离线回放一次 dashboard/WebUI 运行；无需 API key。 |
| `arbor init` | 在目标项目中创建 starter `arbor.yaml`，并可选运行 baseline。 |
| `arbor cost` | 根据计划运行形状或已有 events 文件估算 LLM 花费。 |
...
`arbor demo` 是安装后的低风险检查；`arbor init` 可在正式运行前生成可复用的项目配置；
`arbor cost` 用于粗略估算模型预算，不等同于最终账单。
