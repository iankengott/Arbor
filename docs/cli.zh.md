...
| `arbor demo` | 离线回放一次仪表盘/WebUI 运行，无需 API key。 |
| `arbor init` | 在目标项目中生成 starter `arbor.yaml`，并可运行基线。 |
| `arbor cost` | 根据计划运行形态或现有 events 文件估算 LLM 成本。 |
...
`arbor demo` 是安装后的最快无风险检查。`arbor init` 可选，但适合在接入对话前生成可复用项目配置；如果你的评估命令不是常见名称，可传入 `--eval-cmd "bash eval.sh"`。

`arbor cost` 内置一小张常见 OpenAI 和 Anthropic 模型价格表，也支持私有网关的自定义价格：
...
该估算只是规划工具，不是账单。它无法预知未来重试、确切上下文增长、缓存命中率、输出长度、provider 侧路由或价格变化。
